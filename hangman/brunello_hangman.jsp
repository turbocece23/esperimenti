...
<%
	...
	if(request.getParameter("nuovaParola")!=null)
	{
		...
		{
			lettera=request.getParameter("letteraGET").charAt(0);
		}else
		{
			//Imposta un valore di default in caso la lettera non sia impostata correttamente
			lettera='a';
		}
	}else
	{
		...
		if(request.getParameter("letteraGET")==null)
		{
			...
			Random ran = new Random();
			indice = ran.nextInt(pool.length);
			//Lettera impostata di default ad 'a'
			lettera='a';
		}else
		{
			//Se ciò che è stato inserito è solo una lettera
			if(request.getParameter("letteraGET").length()==1)
			{
...
<html>
	...
	<style type="text/css">
		...
		#bloc1 { display:inline-block; margin-left: 10%; margin-right: 10%; }
			...
			<%
				...
				}else
				{
					...
					if(unalettera==true)
					{
						...
						//Controlla se il carattere (preso dalla richiestaGET) è presente nella parola
				...
				<form method="GET" action="hangman.jsp" autocomplete="off" required>
					<%
						...
						}else
						{
							...
							out.println("<input type=\"text\" name=\"letteraGET\" required=\"true\" placeholder=\"Lettera o parola\"autofocus>");
							out.println("<br>");
							out.println("<br><input class=\"button invia\" type=\"submit\" value=\"Inizia una nuova partita\">");
						}
					%>
					<br>
					<%
						...
						{
							out.println("<input type=\"number\" hidden name=\"indiceGET\" value=\"0\">");
						}
						...
						if(finita==false)
						{
							if(request.getParameter("tentativiGET")!=null)
							{
								...
								out.print("\t\t\t\t\t<span>Lettere già inserite:</span><span style=\"color:red;\"> ");
